...
int ac_power_on(String a){
    
  Serial.println("A variavel a é " + a);
  ...
  Serial.print("A variavel RAW_DATA_LEN é:" + String(RAW_DATA_LEN,DEC));
  
  uint16_t ac_power[RAW_DATA_LEN];

  
  for(int i=0;i<RAW_DATA_LEN;i++){
  ac_power[i] = splitter->getItemAtIndex(i).toInt();
  Serial.println(ac_power[i]);
  }
  irsend.sendRaw(ac_power, RAW_DATA_LEN, 38); //Pass the buffer,length, optionally frequency
  Serial.print(millis());
  ...
  //o reset travou algumas vezes com os pinos D2 e D3 ligados e não travou nenhuma vez com eles desplugados, estudar isso
  if (free < 5000){
    Serial.println("Memoria inferior a 5Kb.\nResetando...");
    //ESP.reset();
    Serial.println("Memoria inferior a 5Kb.\nResetando status das variaveis do aRest...");
    rest.reset_status();
    Serial.println("Memoria inferior a 5Kb.\nResetando Buffer do aRest...");
    //rest.resetBuffer();
  }
...
void setup(void)
{
  ...
  rest.function("ac_power_on",ac_power_on);
  ...
  if (!MDNS.begin("node01")) {             // Start the mDNS responder for esp8266.local
    Serial.println("Error setting up MDNS responder!");
  }
  Serial.println("mDNS responder started at node01.local");
}
